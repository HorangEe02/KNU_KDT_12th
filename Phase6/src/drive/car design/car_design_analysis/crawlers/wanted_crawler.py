"""
=============================================================
원티드 크롤러 - 모빌리티/제조 채용 포지션 크롤링
=============================================================
- Wanted API v4 기반 (requests 사용, Selenium 불필요)
- 직군: 모빌리티/제조 전체 (tag_type_ids=660)
...
class WantedCrawler:
    ...
    # 모빌리티/제조 직군 카테고리
    CATEGORY_ID = 660
    EXTRA_CATEGORY_IDS = [508]

    # 모빌리티/제조 하위 직무 ID→이름 매핑
    # 자동차/모빌리티 하위 직무 ID→이름 매핑 (실제 ID는 원티드 API에서 확인 필요)
    SUBCATEGORY_MAP = {}
    ...
    def fetch_job_listings(self, limit: int = 20) -> list:
        """
        모빌리티/제조 카테고리 전체 포지션 목록을 API로 수집
        ...
        print(f"\n  📋 목록 수집 시작 (카테고리: 모빌리티/제조, ID={self.CATEGORY_ID})")
    ...
    def _run_crawl(self, category_ids, file_suffix="자동차설계"):
    ...
    def run_all(self, extra_category_ids=None):
        """기본 카테고리 + EXTRA_CATEGORY_IDS + 추가 카테고리로 크롤링 실행
        ...
        사용 예시:
            # 기본 카테고리 (660: 모빌리티) + EXTRA (508: 제조/화학)
            crawler.run_all()

            # 기본 + EXTRA + IT/웹/통신 카테고리 추가
            crawler.run_all(extra_category_ids=[506])
        """
        category_ids = [self.CATEGORY_ID] + self.EXTRA_CATEGORY_IDS
        if extra_category_ids:
            category_ids.extend(extra_category_ids)
            print(f"📌 추가 카테고리 {len(extra_category_ids)}개 포함")

        return self._run_crawl(category_ids, file_suffix="자동차설계")

    def run_custom(self, category_ids, file_suffix="자동차설계_추가"):
        ...
        사용 예시:
            ...
            # 여러 카테고리
            crawler.run_custom([506, 507], file_suffix="자동차설계_IT금융")
        """
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="원티드 자동차 설계 채용 포지션 크롤러")
    parser.add_argument(
    ...
    try:
        if args.mode == "all":
            ...
            if not args.extra_category:
                ...
                print("  508: 제조/화학  518: 게임  660: 모빌리티")
            else:
