...
# 기술스택 카테고리 매핑
TECH_CATEGORIES = {
    "CAD 도구": ["CATIA", "NX", "SOLIDWORKS", "AutoCAD", "Creo", "Inventor"],
    "CAE/시뮬레이션": ["ANSYS", "ABAQUS", "HyperMesh", "NASTRAN", "LS-DYNA",
                         "STAR-CCM+", "Fluent", "Adams", "OptiStruct"],
    "프로그래밍/데이터": ["Python", "C++", "C", "MATLAB", "Simulink", "LabVIEW",
                           "Excel", "VBA"],
    "전장/자율주행": ["ECU", "CAN", "LIN", "AUTOSAR", "LiDAR", "레이더",
                        "카메라", "V2X", "BMS"],
    "인프라/PLM": ["Teamcenter", "ENOVIA", "Windchill", "SAP",
                      "AWS", "Azure", "Git", "Docker"],
    "표준/품질": ["GD&T", "ISO", "IATF16949", "DFMEA", "PFMEA",
                    "3D프린팅", "VR", "AR"],
}
...
class CrossAnalysisVisualizer:
    ...
    def plot_tech_category_donut(self):
        ...
        ax.set_title("기술스택 카테고리 분포", fontsize=18, fontweight="bold")
        fig.tight_layout()
    ...
    def plot_field_avg_rating(self):
        ...
        ax.set_xlabel("평균 별점", fontsize=16)
        ax.set_title("기업 분야별 평균 별점", fontsize=20, fontweight="bold")
    ...
    def plot_status_rating_boxplot(self):
        ...
        ax.set_xlabel("")
        ax.set_title("재직상태별 별점 분포", fontsize=20, fontweight="bold")
        ax.tick_params(axis='both', labelsize=14)
        fig.tight_layout()
    ...
    def plot_interview_position(self):
        ...
        for i, val in enumerate(counts.values[::-1]):
            ax.text(val + 0.2, i, str(val), va="center", fontsize=14, fontweight="bold")
        ax.set_xlabel("면접후기 수", fontsize=16)
        ax.set_title("면접 지원직무 분포", fontsize=20, fontweight="bold")
        ax.tick_params(axis='both', labelsize=14)
        fig.tight_layout()
