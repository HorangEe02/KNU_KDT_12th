...
class ChartVisualizer:
    ...
    def plot_tech_stack_bar(self, tech_freq_df: pd.DataFrame):
        ...
        ax.set_title("자동차 설계 채용 기술스택 TOP 15", fontsize=20, fontweight="bold", pad=15)
    ...
    def plot_salary_boxplot(self, salary_data: pd.DataFrame):
        ...
        ax.set_title("자동차 설계 기업별 연봉 분포", fontsize=20, fontweight="bold", pad=15)
        ax.tick_params(axis='both', labelsize=14)
    ...
    def plot_company_ratings_grouped_bar(self, ratings_data: pd.DataFrame):
        ...
        ax.set_title("자동차 설계 기업 평점 항목별 비교", fontsize=20, fontweight="bold", pad=15)
    ...
    def plot_monthly_trend_line(self, monthly_data: pd.DataFrame):
        ...
        ax.set_title("월별 자동차 설계 채용공고 수 추이", fontsize=20, fontweight="bold", pad=15)
        ax.tick_params(axis='both', labelsize=14)
    ...
    def plot_location_bar(self, location_data: pd.DataFrame):
        ...
        ax.set_title("근무 지역별 자동차 설계 채용 분포", fontsize=20, fontweight="bold", pad=15)
        ax.tick_params(axis='both', labelsize=14)
        self._save(fig, "chart_location.png")

    def plot_education_bar(self, education_data: pd.DataFrame):
        """9. 학력 조건 분포 막대그래프 (자동차 설계 특화)"""
        ...
        ax.set_ylabel("채용 공고 수", fontsize=16)
        ax.set_title("학력 조건 분포 (자동차 설계 특화)", fontsize=20, fontweight="bold", pad=15)
        ax.tick_params(axis='both', labelsize=14)
        self._save(fig, "chart_education.png")

    def plot_domain_pie(self, domain_data: pd.DataFrame):
        """10. 자동차 설계 도메인별 채용 분포 파이차트 (자동차 설계 특화)"""
        if domain_data is None or domain_data.empty:
            print("  ⚠️ 자동차 설계 도메인 데이터 없음")
            return
        ...
        for t in autotexts:
            t.set_fontsize(14)
            t.set_fontweight("bold")
        ax.set_title("자동차 설계 도메인별 채용 분포", fontsize=18, fontweight="bold", pad=15)
    ...
    def generate_all(self, data_dict: dict):
        ...
        if data_dict.get("car_design_domain") is not None:
            self.plot_domain_pie(data_dict["car_design_domain"])
