...
class InteractiveVisualizer:
    ...
    def plot_trend_line(self, trend_data: pd.DataFrame):
        ...
        if isinstance(trend_data, pd.DataFrame) and len(trend_data.columns) >= 2:
            ...
            fig = px.line(trend_data, x=x_col, y=y_col,
                          title="자동차 설계 채용 트렌드", template="plotly_white")
    ...
    def plot_domain_sunburst(self, domain_data: pd.DataFrame):
        ...
        if len(domain_data.columns) >= 3:
            ...
            labels = ["자동차 설계"] + domain_data[parent_col].unique().tolist() + domain_data[child_col].tolist()
            parents = [""] + ["자동차 설계"] * domain_data[parent_col].nunique()
            ...
            fig = px.sunburst(
                ...
                title="자동차 설계 도메인 선버스트 차트",
            )

        fig.update_layout(title="자동차 설계 도메인 선버스트 차트", template="plotly_white")
