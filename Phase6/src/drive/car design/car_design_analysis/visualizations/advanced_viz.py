...
class AdvancedVisualizer:
    ...
    # ─────────────────────────────────────────
    # 8. 자동차 설계 도메인-키워드 히트맵
    # ─────────────────────────────────────────
    def plot_domain_keyword_heatmap(self):
        """car_design_domain_frequency 카테고리×키워드 빈도 히트맵"""
        domain_df = self._find_df("car_design_domain")
        if domain_df is None or domain_df.empty:
            print("    ⚠️ 자동차 설계 도메인 데이터 없음 → 도메인 히트맵 스킵")
            return
        ...
        sns.heatmap(pivot, annot=True, fmt=".0f", cmap="YlOrRd",
                    ...
                    annot_kws={"fontsize": 13, "fontweight": "bold"})
        ax.set_title("자동차 설계 도메인별 키워드 빈도 히트맵", fontsize=20, fontweight="bold", pad=15)
        ax.set_ylabel("")
        plt.xticks(rotation=45, ha="right", fontsize=14)
