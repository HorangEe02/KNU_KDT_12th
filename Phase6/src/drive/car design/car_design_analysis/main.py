"""
=============================================================
자동차 설계 분야 취업 동향 분석 - 전체 파이프라인 실행
=============================================================
...
def run_analysis():
    ...
    if not domain_freq.empty:
        domain_freq.to_csv(os.path.join(DATA_PROCESSED_DIR, "car_design_domain_frequency.csv"),
                           index=False, encoding="utf-8-sig")
        print(f"  ✅ 자동차 설계 도메인 빈도 분석 완료: {len(domain_freq)}개")
...
def run_visualization(analysis_results: dict = None):
    ...
    try:
        ...
        wc_data = {"blog": [], "interview": [], "review_pros": [],
                   "review_cons": [], "tech_stack": {}, "car_design_domain": {}}
        ...
        # 자동차 설계 도메인 텍스트
        wc_data["car_design_domain"] = all_texts[:500] if all_texts else []
...
# ============================================================
def main():
    parser = argparse.ArgumentParser(description="자동차 설계 분야 취업 동향 분석")
