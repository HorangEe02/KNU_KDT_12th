...
class DataCleaner:
    ...
    def __init__(self):
        # 자동차 설계 용어 정규화 매핑
        self.car_design_term_map = {
            "CATIA": [r"카티아", r"catia"],
            "NX": [r"엔엑스", r"(?<![A-Za-z])nx(?![A-Za-z])"],
            "SOLIDWORKS": [r"솔리드웍스", r"솔리드\s*웍스", r"solidworks"],
            "ANSYS": [r"안시스", r"ansys"],
            "ABAQUS": [r"아바쿠스", r"abaqus"],
            "HyperMesh": [r"하이퍼메쉬", r"hypermesh"],
            "LS-DYNA": [r"엘에스다이나", r"ls[\-]?dyna", r"lsdyna"],
            "MATLAB": [r"매트랩", r"matlab"],
            "Simulink": [r"시뮬링크", r"simulink"],
            "GD&T": [r"지디앤티", r"기하공차"],
        }
    ...
    def normalize_domain_terms(self, text: str) -> str:
        ...
        result = text
        for standard_term, variants in self.car_design_term_map.items():
