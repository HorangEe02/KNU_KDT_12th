"""
=============================================================
자율주행/모빌리티 분야 취업 동향 분석 - 전체 파이프라인 실행
=============================================================
...
def run_analysis():
    ...
    # 자율주행 도메인 빈도
    domain_freq = analyzer.get_domain_frequency(combined_df)
    if not domain_freq.empty:
        domain_freq.to_csv(os.path.join(DATA_PROCESSED_DIR, "domain_frequency.csv"),
                           index=False, encoding="utf-8-sig")
        print(f"  ✅ 자율주행 도메인 빈도 분석 완료: {len(domain_freq)}개")
...
def run_visualization(analysis_results: dict = None):
    ...
    try:
        ...
        # 자율주행 도메인 텍스트
        wc_data["domain"] = all_texts[:500] if all_texts else []

        # 원티드 채용 자격요건/우대사항/주요업무 텍스트
        job_req_texts = []
        for name, df in all_data.items():
            if "원티드" in name or "wanted" in name.lower():
...
# ============================================================
def main():
    parser = argparse.ArgumentParser(description="자율주행/모빌리티 분야 취업 동향 분석")
    parser.add_argument("--phase", type=str, default="all",
                        choices=["crawl", "process", "analyze", "visualize", "all"],
                        help="실행할 단계 (기본: all)")
    parser.add_argument("--source", type=str, default="all",
                        choices=["naver", "wanted", "jobplanet", "all"],
                        help="크롤링 소스 (기본: all)")
    ...
    print("=" * 60)
    print("🚗 자율주행/모빌리티 분야 취업 동향 분석 시작")
