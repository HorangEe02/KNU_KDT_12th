...
class AdvancedVisualizer:
    ...
    def plot_job_treemap(self):
        ...
        all_jobs = pd.Series(dtype=str)

        # 잡플래닛 리뷰 직종
        review_df = self._find_df("리뷰", "review")
        if review_df is not None and "직종" in review_df.columns:
            all_jobs = pd.concat([all_jobs, review_df["직종"].dropna()], ignore_index=True)

        # 원티드 직무카테고리 (쉼표 분리 멀티카테고리)
        ...
        if wanted_df is not None and "직무카테고리" in wanted_df.columns:
            for val in wanted_df["직무카테고리"].dropna():
                for cat in str(val).split(","):
                    cat = cat.strip()
                    if cat:
                        all_jobs = pd.concat([all_jobs, pd.Series([cat])], ignore_index=True)

        if all_jobs.empty:
            print("    ⚠️ 직종/직무 데이터 없음 → 트리맵 스킵")
            return

        counts = all_jobs.value_counts()
        if counts.empty:
            return
        ...
        ax.set_title("자율주행/모빌리티 직종/직무 분류 트리맵 (잡플래닛+원티드)", fontsize=16, fontweight="bold", pad=15)
        ax.axis("off")
        self._save(fig, "adv_job_treemap.png")

    # ─────────────────────────────────────────
    # 4. 기술스택 네트워크 그래프
    # ─────────────────────────────────────────
    def plot_tech_network(self):
        """블로그+뉴스+원티드 텍스트에서 기술 키워드 동시출현 네트워크"""
        try:
    ...
    def plot_domain_keyword_heatmap(self):
        """domain_frequency 카테고리x키워드 빈도 히트맵"""
        domain_df = self._find_df("domain")
        if domain_df is None or domain_df.empty:
            print("    ⚠️ 자율주행 도메인 데이터 없음 → 도메인 히트맵 스킵")
            return
        ...
        ax.set_title("자율주행 도메인별 키워드 빈도 히트맵", fontsize=16, fontweight="bold", pad=15)
        ax.set_ylabel("")
        plt.xticks(rotation=45, ha="right")
