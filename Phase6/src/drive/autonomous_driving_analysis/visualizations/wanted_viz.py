...
원티드 채용 포지션 시각화 모듈
=============================================================
원티드 자율주행/모빌리티 포지션 데이터를 활용한 시각화 7종 생성
- 자격요건/우대사항 워드클라우드
- 직무카테고리별 분포
- 업종별 분포
- 근무지(지역)별 분포
...
class WantedVisualizer:
    ...
    def plot_requirements_wordcloud(self):
        ...
        ax.set_title("원티드 자율주행/모빌리티 채용 자격요건 워드클라우드",
    ...
    def plot_job_category_dist(self):
        ...
        ax.invert_yaxis()
        ax.set_xlabel("포지션 수", fontsize=12)
        ax.set_title("원티드 직무카테고리별 포지션 분포", fontsize=15, fontweight="bold", pad=15)
    ...
    def plot_industry_dist(self):
        ...
        wedges, texts, autotexts = ax.pie(
            ...
            autopct=lambda pct: f"{pct:.1f}%\n({int(round(pct / 100 * sum(industry_counts.values)))}건)",
            colors=colors,
            startangle=90,
        ...
        ax.set_title("원티드 자율주행/모빌리티 업종별 포지션 분포",
    ...
    def plot_location_dist(self):
        ...
        ax.set_yticklabels(loc_counts.index, fontsize=12)
        ax.invert_yaxis()
        ax.set_xlabel("포지션 수", fontsize=12)
        ax.set_title("원티드 근무지(지역)별 포지션 분포", fontsize=15, fontweight="bold", pad=15)
    ...
    def plot_tech_stack_dist(self):
        ...
        ax.set_title("원티드 자율주행/모빌리티 기술스택 TOP 20",
    ...
    def plot_career_range_dist(self):
        ...
        ax.set_xticklabels(cat_counts.index, fontsize=12)
        ax.set_ylabel("포지션 수", fontsize=12)
        ax.set_title("원티드 자율주행/모빌리티 요구 경력 분포",
                      fontsize=15, fontweight="bold", pad=15)

        for bar, val in zip(bars, cat_counts.values):
            ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 0.3,
                    f"{val}건", ha="center", fontsize=11, fontweight="bold")
    ...
    def plot_company_positions(self):
        ...
        ax.set_yticklabels(company_counts.index, fontsize=11)
        ax.invert_yaxis()
        ax.set_xlabel("포지션 수", fontsize=12)
        ax.set_title("원티드 자율주행/모빌리티 회사별 채용 포지션 수 TOP 15",
                      fontsize=15, fontweight="bold", pad=15)
