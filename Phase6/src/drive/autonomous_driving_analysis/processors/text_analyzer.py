...
class TextAnalyzer:
    ...
    def __init__(self):
        ...
        # 자율주행 도메인 사용자 사전
        self.domain_terms = [
            "자율주행", "센서퓨전", "객체인식", "경로계획", "포인트클라우드",
            "커넥티드카", "모빌리티", "시뮬레이션", "디지털트윈",
            "차량제어", "모션플래닝", "차량통신",
        ]
    ...
    def get_domain_frequency(self, job_data: pd.DataFrame) -> pd.DataFrame:
        """
        자율주행 도메인 키워드 빈도 분석 (자율주행 특화)

        카테고리: 인지 / 판단 / 제어 / 인프라 / 시뮬레이션
...
if __name__ == "__main__":
    ...
    # 테스트
    test_text = "Python, C++, ROS2 활용한 자율주행 LiDAR 센서퓨전 개발 경력 3~5년"
