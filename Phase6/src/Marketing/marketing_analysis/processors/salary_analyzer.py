...
class SalaryAnalyzer:
        ...
        if self.salary_col is None:
            for col in self.data.columns:
                if "연봉" in col or "salary" in col.lower():
                    ...
                    break
        if self.salary_col:
            self.data[self.salary_col] = pd.to_numeric(self.data[self.salary_col], errors="coerce")
    ...
    def get_salary_stats(self) -> dict:
        ...
        return {
            "평균": round(salary.mean(), 1), "중앙값": round(salary.median(), 1),
            "최소": round(salary.min(), 1), "최대": round(salary.max(), 1),
    ...
    def get_salary_by_tech_stack(self) -> pd.DataFrame:
        ...
        results = []
        target_techs = [
            "Google Analytics", "GA4", "Google Ads", "Facebook Ads",
            "SQL", "Python", "Tableau", "HubSpot", "SEO", "Amplitude",
        ]
