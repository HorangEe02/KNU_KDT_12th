...
class TextAnalyzer:
    ...
    def __init__(self):
        ...
        try:
            ...
            print(f"  KoNLPy 사용 불가 ({e}), 정규식 기반 추출 사용")
        ...
        self.domain_terms = [
            "디지털마케팅", "퍼포먼스마케팅", "콘텐츠마케팅", "브랜드마케팅",
            "그로스해킹", "마케팅자동화", "리타겟팅", "프로그래매틱",
            "인플루언서마케팅", "소셜미디어마케팅", "이메일마케팅",
        ]
    ...
    def extract_nouns(self, text: str) -> list:
        ...
        if self.okt:
            ...
            nouns = self._regex_extract_nouns(text)
        filtered = [n for n in nouns if len(n) > 1 and n not in self.stopwords]
        return filtered
    ...
    def get_tech_stack_frequency(self, job_data: pd.DataFrame) -> pd.DataFrame:
        ...
        if not counter:
            return pd.DataFrame(columns=["기술스택", "빈도", "비율(%)"])
        total = sum(counter.values())
        data = [(tech, freq, round(freq / total * 100, 2)) for tech, freq in counter.most_common()]
    ...
    def get_certificate_frequency(self, job_data: pd.DataFrame) -> pd.DataFrame:
        ...
        if not counter:
            return pd.DataFrame(columns=["자격증", "빈도", "비율(%)"])
        total = sum(counter.values())
        data = [(cert, freq, round(freq / total * 100, 2)) for cert, freq in counter.most_common()]
    ...
    def get_domain_frequency(self, job_data: pd.DataFrame) -> pd.DataFrame:
        ...
        combined = " ".join(str(t) for t in texts)
        results = []
        for category, keywords in MARKETING_DOMAIN_KEYWORDS.items():
            for keyword in keywords:
                pattern = re.compile(re.escape(keyword), re.IGNORECASE)
                count = len(pattern.findall(combined))
                if count > 0:
                    results.append({"카테고리": category, "키워드": keyword, "빈도": count})
...
if __name__ == "__main__":
    ...
    test_text = "GA4, Google Ads, SQL 활용 퍼포먼스 마케팅 경력 3~5년 ROAS 분석"
