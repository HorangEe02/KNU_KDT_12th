...
# 기술스택 카테고리 분류 (마케팅 특화)
TECH_CATEGORIES = {
    "광고 플랫폼": ["Google Ads", "Facebook Ads", "Instagram Ads", "네이버광고",
                     "카카오모먼트", "TikTok Ads", "LinkedIn Ads"],
    "분석/데이터": ["Google Analytics", "GA4", "Amplitude", "Mixpanel",
                     "SQL", "Python", "R", "Tableau", "Power BI", "Looker Studio"],
    "마케팅 자동화": ["HubSpot", "Salesforce", "Marketo", "Braze",
                       "Mailchimp", "Pardot"],
    "SEO/콘텐츠": ["SEO", "SEM", "Google Search Console", "Ahrefs",
                     "SEMrush", "WordPress", "콘텐츠마케팅"],
    "CRM/고객관리": ["CRM", "Zendesk", "채널톡",
                      "고객세그먼트", "리텐션"],
    "소셜/콘텐츠 도구": ["Figma", "Canva", "Photoshop", "Premiere Pro",
...
class AdvancedVisualizer:
    ...
    def plot_keyword_tech_heatmap(self):
        ...
        ax.set_title("검색키워드별 기술스택 언급 히트맵", fontsize=16, fontweight="bold", pad=15)
        ax.set_ylabel("")
    ...
    def plot_company_radar(self):
        ...
        ax.set_ylim(0, 5.5)
        ax.set_title("기업 종합 프로필 레이더차트", fontsize=16, fontweight="bold", pad=25)
    ...
    def _plot_single_treemap(self, jobs_series, title, filename, top_n=20, colormap="Set3"):
        ...
        fontsize = 11 if len(counts) <= 15 else 9
    ...
    def plot_tech_network(self):
        ...
        ax.set_title("기술스택 동시출현 네트워크", fontsize=16, fontweight="bold", pad=15)
        ax.axis("off")
    ...
    def plot_tech_category_stacked_bar(self):
        ...
        ax.set_title("기술스택 카테고리별 상세 분포", fontsize=16, fontweight="bold", pad=15)
    ...
    def plot_review_violin(self):
        ...
        sns.violinplot(data=df, x="기업명", y="별점", ax=ax, palette="Set2",
                       inner="box", cut=0)
        ax.set_xlabel("")
        ax.set_ylabel("별점 (5점 만점)", fontsize=12)
        ax.set_ylim(0, 5.5)
        ax.set_title("기업별 리뷰 별점 분포 (바이올린 플롯)", fontsize=16, fontweight="bold", pad=15)
    ...
    def plot_keyword_bubble(self):
        ...
        ax.set_xticklabels([f"{m.month}월" for m in all_months], rotation=45, ha="right", fontsize=9)
        ax.set_yticks(range(len(top_kw)))
        ax.set_yticklabels(top_kw, fontsize=9)
        ax.set_xlabel("월", fontsize=11)
        ax.set_ylabel("검색키워드", fontsize=11)
        ax.set_title("검색키워드별 월별 블로그 게시 버블차트", fontsize=16, fontweight="bold", pad=15)
    ...
    def plot_domain_keyword_heatmap(self):
        """marketing_domain_frequency 카테고리×키워드 빈도 히트맵"""
        domain_df = self._find_df("marketing_domain")
        if domain_df is None or domain_df.empty:
            print("    ⚠️ 마케팅 도메인 데이터 없음 → 도메인 히트맵 스킵")
            return
        ...
        ax.set_title("마케팅 도메인별 키워드 빈도 히트맵", fontsize=16, fontweight="bold", pad=15)
        ax.set_ylabel("")
        plt.xticks(rotation=45, ha="right")
    ...
    def plot_company_interview_comparison(self):
        ...
        ax1.set_xlabel("면접 건수", fontsize=11, color="#5DADE2")
        ax1.set_yticks(y)
        ax1.set_yticklabels(grouped.index, fontsize=10)
        for bar, val in zip(bars, grouped["면접건수"]):
            ax1.text(val + 0.2, bar.get_y() + bar.get_height() / 2,
                     str(int(val)), va="center", fontsize=9, fontweight="bold", color="#5DADE2")

        ax2 = ax1.twiny()
        ax2.plot(grouped["평균난이도"], y, "D-", color="#E74C3C", markersize=8, linewidth=2, label="평균 난이도")
        ax2.set_xlabel("평균 면접난이도 (1~5)", fontsize=11, color="#E74C3C")
        ax2.set_xlim(0, 5.5)
        ...
        ax1.set_title("기업별 면접 건수 & 난이도 비교", fontsize=16, fontweight="bold", pad=40)
        fig.tight_layout()
    ...
    def plot_word_freq_bubble(self):
        ...
        for i, (_, row) in enumerate(top.iterrows()):
            fs = max(8, int(sizes[i] / max_size * 16))
        ...
        ax.set_ylim(-1, 11)
        ax.axis("off")
        ax.set_title("핵심 키워드 버블 시각화 (TOP 30)", fontsize=16, fontweight="bold", pad=15)
