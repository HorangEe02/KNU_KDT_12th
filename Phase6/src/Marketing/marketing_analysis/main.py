...
def run_analysis():
    ...
    if not domain_freq.empty:
        domain_freq.to_csv(os.path.join(DATA_PROCESSED_DIR, "marketing_domain_frequency.csv"),
                           index=False, encoding="utf-8-sig")
        print(f"  ✅ 마케팅 도메인 빈도 분석 완료: {len(domain_freq)}개")
...
def run_visualization(analysis_results: dict = None):
    ...
    try:
        ...
        wc_data = {"blog": [], "interview": [], "review_pros": [],
                   "review_cons": [], "tech_stack": {}, "marketing_domain": []}
        ...
        # 마케팅 도메인 텍스트
        wc_data["marketing_domain"] = all_texts[:500] if all_texts else []
...
# ============================================================
def main():
    parser = argparse.ArgumentParser(description="마케팅 분야 취업 동향 분석")
