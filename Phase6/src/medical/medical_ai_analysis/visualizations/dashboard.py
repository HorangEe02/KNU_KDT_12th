...
class AnalysisDashboard:
    ...
    def _get_domain_freq(self):
        """의료 도메인 빈도"""
        return self._find_df("medical_domain")
    ...
    def _draw_tech_stack(self, ax):
        ...
        ax.set_title("기술스택 TOP 10", fontsize=18, fontweight="bold")
    ...
    def _draw_review_status(self, ax):
        ...
        ax.set_title("리뷰 재직상태 분포", fontsize=18, fontweight="bold")
    ...
    def _draw_interview_difficulty(self, ax):
        ...
        if interview_df is not None and "면접난이도" in interview_df.columns:
            ...
            ax.tick_params(axis='both', labelsize=12)
        ax.set_title("면접 난이도 분포", fontsize=18, fontweight="bold")
    ...
    def _draw_company_ratings(self, ax):
        ...
        if review_df is not None and "별점" in review_df.columns:
            ...
            for i, (name, val) in enumerate(company_avg.items()):
                ax.text(val + 0.05, i, f"{val:.1f}", va="center", fontsize=13, fontweight="bold")
            ax.legend(fontsize=12)
            ax.tick_params(axis='both', labelsize=12)
        ax.set_title("기업 평균 별점 비교", fontsize=18, fontweight="bold")
    ...
    def _draw_monthly_trend(self, ax):
        ...
        if month_labels and month_values:
            ...
            ax.set_ylabel("콘텐츠 수", fontsize=14)
            plt.sca(ax)
            plt.xticks(rotation=45, ha="right", fontsize=12)
        ax.set_title("월별 의료AI 콘텐츠 동향 (최근 12개월)", fontsize=18, fontweight="bold")
    ...
    def _draw_company_size(self, ax):
        ...
        ax.set_title("기업 규모 분포", fontsize=18, fontweight="bold")

    def _draw_medical_domain(self, ax):
        """[7] 의료 도메인 분포"""
        ...
        ax.set_title("의료 도메인 분포", fontsize=18, fontweight="bold")
        ...
        if cert_df is not None and not cert_df.empty:
            ...
            for bar, val in zip(bars, cert_df[freq_col]):
                ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 0.2,
                        str(int(val)), ha="center", fontsize=13, fontweight="bold")
            ax.set_ylabel("빈도", fontsize=14)
            plt.sca(ax)
            plt.xticks(rotation=30, ha="right", fontsize=12)
        ax.set_title("자격증/우대사항 빈도", fontsize=18, fontweight="bold")
    ...
    def _draw_key_stats(self, ax):
        ...
        ax.text(0.05, 0.95, stat_text, transform=ax.transAxes, fontsize=14,
        ...
        ax.set_title("핵심 통계", fontsize=18, fontweight="bold")
    ...
    def save_individual_charts(self):
        ...
        chart_specs = [
            ...
            (self._draw_medical_domain,      "dashboard_07_medical_domain.png",      (7, 7)),
    ...
    def create_summary_dashboard(self):
        ...
        fig.suptitle("의료 AI 분야 취업 동향 종합 분석",
